"""
ReguSim Backend - Flask Application Factory
"""
...
def create_app(config_class=Config):
    ...
    if should_log_startup:
        logger.info("=" * 50)
        logger.info("ReguSim Backend starting...")
        logger.info("=" * 50)
    ...
    @app.before_request
    def log_request():
        logger = get_logger('regusim.request')
    ...
    @app.route('/health')
    def health():
        return {'status': 'ok', 'service': 'ReguSim Backend'}

    if should_log_startup:
        logger.info("ReguSim Backend startup complete")
