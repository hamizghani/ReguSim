...
logger = get_logger('regusim.oasis_profile')
...
class OasisProfileGenerator:
    ...
    COUNTRIES = [
        ...
        "Singapore", "Malaysia", "US", "UK", "Japan", "Australia"
    ]

    # Individual type entities (need to generate specific personas)
    INDIVIDUAL_ENTITY_TYPES = [
        "regulator", "policymaker", "bankexecutive", "fintechfounder",
        "economist", "consumeradvocate", "journalist", "auditor",
        "person", "publicfigure", "expert", "official", "activist"
    ]

    # Group/institutional type entities (need to generate group representative personas)
    GROUP_ENTITY_TYPES = [
        "regulatorybody", "bank", "fintechcompany", "industryassociation",
        "governmentagency", "mediaoutlet", "consumergroup",
        "organization", "institution", "ngo", "company", "community"
    ]
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating stakeholder profiles for regulatory impact assessment simulation in the Indonesian financial sector. Generate detailed, realistic personas for regulatory stakeholders that accurately reflect real-world institutional roles and behaviors. Must return valid JSON format with all string values containing no unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed stakeholder persona for the entity in the context of Indonesian financial regulatory impact assessment.
...
1. bio: Professional bio, 200 characters
2. persona: Detailed persona description (2000 words of pure text), must include:
   - Basic information (age, profession, educational background, location in Indonesia or relevant country)
   - Professional background (role in financial regulation, institutional affiliation, policy expertise)
   - Personality traits (MBTI type, core personality, decision-making style)
   - Communication behavior (public statement patterns, preferred channels, stakeholder engagement style)
   - Regulatory positions and views (attitudes toward financial innovation vs. consumer protection, risk appetite)
   - Unique features (known policy positions, regulatory track record, industry reputation)
   - Institutional memory (involvement in past regulatory events, policy decisions, enforcement actions)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed institutional stakeholder profile for an entity participating in Indonesian financial regulatory impact assessment.
...
1. bio: Official institutional profile, 200 characters, professional and appropriate
2. persona: Detailed institutional profile description (2000 words of pure text), must include:
   - Basic institutional information (official name, organizational type, founding mandate, regulatory jurisdiction)
   - Institutional positioning (role in financial ecosystem, regulatory authority, target stakeholders)
   - Communication style (official language register, policy announcement patterns, stakeholder engagement approach)
   - Policy behavior (regulatory stance, enforcement patterns, consultation processes)
   - Position and attitude (stance on financial innovation, consumer protection priorities, risk tolerance)
   - Institutional context (role in Indonesian financial regulation, relationships with OJK/BI/Kemenkeu)
   - Institutional memory (involvement in past regulatory actions, licensing decisions, policy changes)
...
- Institutional account speech must match its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["regulator", "policymaker", "official"]:
            return {
                "bio": f"Senior {entity_type.lower()} in Indonesian financial regulation.",
                "persona": f"{entity_name} is a {entity_type.lower()} engaged in Indonesian financial regulatory policy. They provide guidance on compliance, risk management, and financial innovation oversight.",
                "age": random.randint(40, 60),
                "gender": random.choice(["male", "female"]),
                "mbti": random.choice(["ENTJ", "ISTJ", "INTJ", "ESTJ"]),
                "country": "Indonesia",
                "profession": "Financial Regulator",
                "interested_topics": ["Financial Regulation", "Consumer Protection", "Systemic Risk"],
            }

        elif entity_type_lower in ["bankexecutive", "fintechfounder", "economist", "expert"]:
            return {
                "bio": f"Expert and thought leader in Indonesian financial services.",
                "persona": f"{entity_name} is a recognized {entity_type.lower()} who provides insights on financial regulation, market dynamics, and policy impact in Indonesia's financial sector.",
                "age": random.randint(35, 55),
                "gender": random.choice(["male", "female"]),
                "mbti": random.choice(["ENTJ", "INTJ", "ENTP", "INTP"]),
                "country": "Indonesia",
                "profession": entity_attributes.get("occupation", "Financial Expert"),
                "interested_topics": ["Financial Innovation", "Regulatory Compliance", "Market Development"],
            }

        elif entity_type_lower in ["consumeradvocate", "journalist", "auditor", "activist"]:
            return {
                "bio": f"{entity_type} focused on Indonesian financial sector accountability.",
                "persona": f"{entity_name} is a {entity_type.lower()} who monitors and reports on financial regulation, consumer protection, and institutional accountability in Indonesia.",
                "age": random.randint(30, 50),
                "gender": random.choice(["male", "female"]),
                "mbti": random.choice(["ENFJ", "INFJ", "ENTP", "INTP"]),
                "country": "Indonesia",
                "profession": entity_type,
                "interested_topics": ["Consumer Protection", "Financial Transparency", "Public Policy"],
            }

        elif entity_type_lower in ["mediaoutlet"]:
            return {
                "bio": f"Official account for {entity_name}. Financial news and regulatory updates.",
                "persona": f"{entity_name} is a media entity that reports on financial regulation, policy changes, and market developments in the Indonesian financial sector.",
                "age": 30,
                "gender": "other",
                "mbti": "ISTJ",
                "country": "Indonesia",
                "profession": "Financial Media",
                "interested_topics": ["Financial News", "Regulatory Updates", "Market Analysis"],
            }

        elif entity_type_lower in ["regulatorybody", "bank", "fintechcompany", "industryassociation", "governmentagency", "consumergroup", "ngo", "organization"]:
            return {
                "bio": f"Official account of {entity_name}.",
                "persona": f"{entity_name} is an institutional stakeholder in Indonesian financial regulation that communicates official positions, policy responses, and engages with regulators and the public on regulatory matters.",
                "age": 30,
                "gender": "other",
                "mbti": "ISTJ",
                "country": "Indonesia",
                "profession": entity_type,
                "interested_topics": ["Financial Regulation", "Policy Compliance", "Industry Development"],
            }
            ...
            return {
                ...
                "persona": entity_summary or f"{entity_name} is a {entity_type.lower()} participating in Indonesian financial regulatory discourse.",
                "age": random.randint(25, 50),
                "gender": random.choice(["male", "female"]),
                "mbti": random.choice(self.MBTI_TYPES),
                "country": "Indonesia",
                "profession": entity_type,
                "interested_topics": ["Financial Regulation", "Policy Impact"],
            }
